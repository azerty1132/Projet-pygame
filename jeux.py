...
y_plateforme2 = 400# Coordonnée y de la plateforme
...
tous_les_sprites.add(joueur, plateforme)  # Ajoute la plateforme au groupe de sprites
...
while continuer:
    ...
    if pygame.sprite.collide_rect(joueur, plateforme,plateforme2):
    ...
    if pygame.sprite.collide_rect(joueur, plateforme2):
        if joueur.rect.bottom <= plateforme2.rect.top and joueur.vitesse_y >= 0:  # Vérifier si le bas du joueur est au-dessus du haut de la deuxième plateforme et si le joueur descend
            if pygame.key.get_pressed()[pygame.K_SPACE]:  # Vérifier si le joueur appuie sur la touche de saut
                joueur.sauter()
            else:
